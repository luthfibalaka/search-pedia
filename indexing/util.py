class IdMap:
    ...
    def __len__(self):
        ...
        return len(self.str_to_id.keys())
...
def merge_and_sort_posts_and_tfs(posts_tfs1: list[tuple], posts_tfs2: list[tuple]):
    """
    ...
    """
    combined_posts_tfs: list[tuple] = []

    posts_tfs1_idx = 0
    posts_tfs2_idx = 0

    while posts_tfs1_idx != len(posts_tfs1) and posts_tfs2_idx != len(posts_tfs2):
        doc_id1 = posts_tfs1[posts_tfs1_idx][0]
        doc_id2 = posts_tfs2[posts_tfs2_idx][0]
        tf1 = posts_tfs1[posts_tfs1_idx][1]
        tf2 = posts_tfs2[posts_tfs2_idx][1]

        if doc_id1 < doc_id2:
            combined_posts_tfs.append((doc_id1, tf1))
            posts_tfs1_idx += 1
        elif doc_id2 < doc_id1:
            combined_posts_tfs.append((doc_id2, tf2))
            posts_tfs2_idx += 1
        else:
            combined_posts_tfs.append((doc_id1, tf1 + tf2))
            posts_tfs1_idx += 1
            posts_tfs2_idx += 1

    while posts_tfs1_idx != len(posts_tfs1):
        doc_id1 = posts_tfs1[posts_tfs1_idx][0]
        tf1 = posts_tfs1[posts_tfs1_idx][1]
        combined_posts_tfs.append((doc_id1, tf1))
        posts_tfs1_idx += 1

    while posts_tfs2_idx != len(posts_tfs2):
        doc_id2 = posts_tfs2[posts_tfs2_idx][0]
        tf2 = posts_tfs2[posts_tfs2_idx][1]
        combined_posts_tfs.append((doc_id2, tf2))
        posts_tfs2_idx += 1

    return combined_posts_tfs
