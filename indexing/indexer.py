...
class InvertedIndex:
    ...
    postings_dict: Dictionary mapping:
        ...
        yang memetakan term ID (integer) ke 4-tuple:
           1. start_position_in_index_file : (dalam satuan bytes) posisi di mana
    ...
    def __enter__(self):
        ...
        """
        # Membuka index file
        self.index_file = open(os.path.join(os.path.join(os.path.join(os.path.dirname(__file__), self.index_file_path))), "rb+")

        # Kita muat postings dict dan terms iterator dari file metadata
        with open(os.path.join(os.path.join(os.path.dirname(__file__), self.metadata_file_path)), "rb") as f:
            (
                self.postings_dict,
                self.terms,
                self.doc_length,
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        with open(os.path.join(os.path.join(os.path.dirname(__file__), self.metadata_file_path)), "wb") as f:
            pickle.dump(
                [self.postings_dict, self.terms, self.doc_length, self.avg_doc_length],
                f,
            )
...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term: int):
        """
        ...
        """
        term_information = self.postings_dict[term]
        self.index_file.seek(term_information[0])

        postings_list: list[int] = self.postings_encoding.decode(
            self.index_file.read(term_information[2])
        )
        tfs_list: list[int] = self.postings_encoding.decode_tf(
            self.index_file.read(term_information[3])
        )
        return (postings_list, tfs_list)
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term: int, postings_list: list[int], tf_list: list[int]):
        """
        ...
        """
        # 1. Encode postings_list dan tfs_list
        encoded_postings_list = self.postings_encoding.encode(postings_list)
        encoded_tfs_list = self.postings_encoding.encode_tf(tf_list)

        # 2. Menyimpan metadata
        self.terms.append(term)

        self.index_file.seek(0, 2)  # Ke akhir file untuk append
        ...
        length_in_bytes_of_tf_list = len(encoded_tfs_list)
        ...
        for i in range(len(postings_list)):
            ...
            if posting in self.doc_length:
                self.doc_length[posting] += tf_list[i]
            else:
                self.doc_length[posting] = tf_list[i]

        # 3. Append bytestream ke index file
        self.index_file.write(encoded_postings_list)
        self.index_file.write(encoded_tfs_list)
