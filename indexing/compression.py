...
class VBEPostings:
    ...
    def vb_encode_number(number):
        ...
        """
        encoded_number = 0
        shifting_factor = 0  # Untuk shifting saat concatenate bits
        while True:
            encoded_number = (number % 128) << (8 * shifting_factor) | encoded_number
            if number < 128:
                break
            number = number // 128
            shifting_factor += 1
        encoded_number = 0b10000000 | encoded_number  # flip MSB of the rightmost byte
        return int(encoded_number).to_bytes(shifting_factor + 1, "big")
    ...
    def vb_encode(list_of_numbers):
        ...
        """
        vb_encoded_list = bytearray()
        for number in list_of_numbers:
            vb_encoded_list.extend(VBEPostings.vb_encode_number(number))
        return vb_encoded_list
    ...
    def encode(postings_list):
        ...
        """
        gaps_list: list[int] = []
        gaps_list.append(postings_list[0])
        for i in range(1, len(postings_list)):
            gaps_list.append(postings_list[i] - postings_list[i - 1])
        return VBEPostings.vb_encode(gaps_list)
    ...
    def decode(encoded_postings_list):
        ...
        for i in range(1, len(gaps_list)):
            postings_list.append(gaps_list[i] + postings_list[i - 1])
